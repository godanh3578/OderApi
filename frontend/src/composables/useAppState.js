...
export function toggleWishlist(product) {
  ...
  if (wishlist.value.includes(id)) {
    ...
    showNotice('Đã xóa khỏi danh sách yêu thích.')
  } else {
...
function createLocalDemoCustomer(profile) {
  return {
    customerId: demoIdCounter--, fullName: profile.fullName, phone: profile.phone,
    email: profile.email || '', address: profile.address || '', gender: profile.gender,
    dateOfBirth: profile.dateOfBirth, currentDebt: 0, totalSpent: 0, membershipTier: 'Thường', walletBalance: 500000
  }
}
...
export async function registerCustomer() {
  ...
  try {
    ...
    const res = await api.post('/api/Customers', {
      fullName: registerForm.value.fullName.trim(), phone: registerForm.value.phone.trim(),
      email: registerForm.value.email.trim(), address: registerForm.value.address.trim(),
...
export async function loginCustomer() {
  authBusy.value = true; authError.value = ''
  if (!loginForm.value.phone.trim()) { authError.value = 'Vui lòng nhập số điện thoại.'; authBusy.value = false; return }
  if (!verifyDemoPassword(loginForm.value.phone.trim(), loginForm.value.password)) { authError.value = 'Mật khẩu không đúng.'; authBusy.value = false; return }
  try {
...
export async function handleFileChange(event) {
  ...
  } catch { showNotice('Không thể tải ảnh lên. Vui lòng thử lại.') }
}
...
export async function submitCheckout() {
  ...
  try {
    ...
    const payload = {
      customerId: customer.customerId, discountAmount: discountAmount.value,
      paymentMethod: backendPaymentMethod(checkout.value.paymentMethod), paidAmount: paidAmount.value,
...
export async function saveSupplier() {
  ...
  try {
    if (supplierEditingId.value) {
      ...
      showNotice('Đã cập nhật nhà cung cấp.')
    } else {
      ...
      showNotice('Đã thêm nhà cung cấp.')
    }
    resetSupplierForm()
    await loadStaffData()
...
export async function deleteSupplier(supplier) {
  ...
  try {
    ...
    await loadStaffData()
    showNotice('Đã xóa nhà cung cấp.')
...
export async function payDebt(debt) {
  ...
  try {
    ...
    await loadStaffData()
    showNotice('Đã ghi nhận thanh toán công nợ.')
...
export async function markOrderPaid(order) {
  ...
  if (order.isLocalDemo || String(id).startsWith('local-')) {
    ...
    upsertLocalOrder(updated)
    await loadMyOrders()
    showNotice('Đã gửi thông báo đến nhân viên. Vui lòng chờ xác nhận.')
    return
  }
  try {
...
export async function approveOrder(order) {
  ...
  showNotice('Đã duyệt đơn. Đang chuẩn bị hàng.')
}
...
export async function completeOrderDelivery(order) {
  ...
  if (order.isLocalDemo || String(id).startsWith('local-')) {
    ...
    await loadMyOrders(); await loadStaffData()
    showNotice('Đã hoàn thành đơn hàng.')
    return
  }
